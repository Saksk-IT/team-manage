...
- 新增：质保到期自动清理结果写入后台自动清理记录，并展示清理来源、停用白名单邮箱与处理原因。
- 修复：质保到期自动踢出在空跑后会按最近到期订单缩短下一轮检查，并在质保时间/次数保存或批量修改后立即唤醒清理任务。
- 修复：质保兑换码成功入列时立即记录对应 Team ID，避免订单到期后因缺少 Team 关联而无法自动清理。
- 修复：同邮箱同 Team 仍有有效质保订单时，过期订单完成标记后不再被成员快照重复捞取。
- 新增：系统设置新增“质保到期自动踢出”开关，开启后自动清理到期质保订单对应 Team 成员并停用邮箱白名单。
- 新增：新增/编辑质保弹窗支持按时分秒设置剩余时间，保存精确到秒。
- 新增：质保选项列表展示“质保到期时间”，剩余时间改为精确到秒。
