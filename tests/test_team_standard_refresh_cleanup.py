...
from app.models import RedemptionCode, Team, TeamCleanupRecord, TeamMemberSnapshot, WarrantyEmailEntry
...
class TeamStandardRefreshCleanupTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_admin_added_warranty_member_enters_manual_whitelist(self):
        async with self.Session() as session:
            ...
            entry = await session.scalar(
                select(WarrantyEmailEntry).where(WarrantyEmailEntry.email == "manual-added@example.com")
            )
