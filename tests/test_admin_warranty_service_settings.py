...
from app.routes.admin import (
    RedeemServiceSettingsRequest,
    WarrantyServiceSettingsRequest,
    update_redeem_service_settings,
...
class AdminRedeemServiceSettingsTests(unittest.IsolatedAsyncioTestCase):
    async def test_update_redeem_service_settings_returns_success(self):
        db = AsyncMock()

        with patch(
            "app.routes.admin.settings_service.update_redeem_service_config",
            new=AsyncMock(return_value=True)
        ) as mocked_update:
            response = await update_redeem_service_settings(
                redeem_data=RedeemServiceSettingsRequest(enabled=True),
                db=db,
                current_user={"username": "admin"}
            )
    ...
    async def test_update_redeem_service_settings_returns_500_on_failure(self):
        db = AsyncMock()
        ...
        with patch(
            ...
            response = await update_redeem_service_settings(
                redeem_data=RedeemServiceSettingsRequest(enabled=False),
                db=db,
                current_user={"username": "admin"}
            )
