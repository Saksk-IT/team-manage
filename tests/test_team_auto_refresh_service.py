...
class TeamAutoRefreshServiceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_run_once_records_unhandled_refresh_exception_and_resets_timer(self):
        fd, db_path = tempfile.mkstemp(suffix=".db")
        os.close(fd)
        engine = create_async_engine(f"sqlite+aiosqlite:///{db_path}", future=True)
        Session = async_sessionmaker(engine, expire_on_commit=False)
        ...
        try:
            ...
            async with Session() as session:
                team = Team(
                    ...
                    access_token_encrypted="enc-exception",
                    account_id="acc-exception",
                    team_type="standard",
                    status="active",
                    current_members=1,
                    max_members=5,
            ...
            service = TeamAutoRefreshService()
            with (
                patch("app.services.team_auto_refresh.AsyncSessionLocal", Session),
                patch.object(
                    service,
                    "_get_runtime_config",
                    new=AsyncMock(return_value={"enabled": True, "interval_minutes": 5})
                ...
                patch(
                    "app.services.team_auto_refresh.team_service.refresh_team_state",
                    new=AsyncMock(side_effect=RuntimeError("sensitive marker should stay out of record"))
                ),
            ):
                interval = await service.run_once()
            ...
            self.assertEqual(refresh_record.source, "auto")
            self.assertEqual(refresh_record.refresh_status, "failed")
            self.assertEqual(refresh_record.error_code, "auto_refresh_exception")
            self.assertIn("RuntimeError", refresh_record.error)
            self.assertNotIn("sensitive marker", refresh_record.error)
        finally:
            await engine.dispose()
            if os.path.exists(db_path):
                os.remove(db_path)
