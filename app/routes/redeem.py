...
async def confirm_redeem(
    ...
    try:
        ...
        if not result["success"]:
            ...
            if any(kw in error_msg for kw in ["不存在", "已使用", "已过期", "截止时间", "已满", "席位", "质保", "无效", "失效", "处理中", "正在被其他邮箱处理", "maximum number of seats", "绑定的 Team", "固定 Team", "没有可用", "号池 Team"]):
                status_code = status.HTTP_400_BAD_REQUEST
                if any(kw in error_msg for kw in ["已满", "席位", "maximum number of seats", "没有可用", "号池 Team"]):
